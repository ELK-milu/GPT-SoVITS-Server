...
import GPT_SoVITS.module.cnhubert as cnhubert
...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language):
    ...
    for text in texts:
        ...
        if prompt_language in usable_langs:
        ...
        if text_language in usable_langs:
...
def handle(command,  text, text_language='zh'): 
    ...
    # 生成唯一文件名
    import uuid
    filename = f"output_{uuid.uuid4().hex[:6]}.wav"
    save_path = os.path.join("audio_output", filename)
...
async def list_audio():
    return {"files": os.listdir("audio_output")}
...
if __name__ == "__main__":
    uvicorn.run(app='SoVits-server:app',host='0.0.0.0', port=8080, workers=1)